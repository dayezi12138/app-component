...
public class TitleItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public TitleItemDecoration(Context context, List<FriendInfo> data) {
        super();
        mData = data;
        mPaint = new Paint();
        mBounds = new Rect();
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (position > -1) {
                if (position == 0) {//等于0的时候绘制title
                    ...
                    int d = position, b = position - 1;
//                    if (mData.size() == position) {
//                        d = position - 1;
//                        b = d-1;
//                    }
                    if (mData.size() == position) return;
                    if (null != mData.get(d).getLetters() && !mData.get(d)
                            .getLetters().equals(mData.get(b).getLetters())) {
...
//            d = position - 1;
//        }
        mPaint.getTextBounds(mData.get(d).getLetters(), 0, mData.get(d).getLetters().length(), mBounds);
        c.drawText(mData.get(d).getLetters(),
...
//                }
                if (mData.size() == position) return;
                if (null != mData.get(d).getLetters() &&
                        !mData.get(d).getLetters().equals(mData.get(b).getLetters())) {
                    //字母不为空，并且不等于前一个，绘制title
                    outRect.set(0, mTitleHeight, 0, 0);
                } else {
                    outRect.set(0, 0, 0, 0);
    ...
    public void setData(List<FriendInfo> mData) {
        this.mData = mData;
    }
}